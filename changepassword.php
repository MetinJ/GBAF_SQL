...
if (isset($_SESSION['secret_question'])) $secretQuestion = $_SESSION
['secret_question'];
...
// Find id of the user:
$req = $connection->prepare('SELECT * FROM accounts WHERE username=? AND 
secret_question=? AND answer=?');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  if (isset($_POST['recover'])) {
    ...
    try {
      ...
      if (strlen($_POST['username']) < 2 || strlen($_POST['username']) > 
      10) {
...
?>
  ...
  <form class="form-box" action="" method="POST">
    ...
    ?>
    <label for="lastname">Nouveau nom(Entre 2 à 10 caractères): </label>
    <input type="text" name="lastname"><br>
    <label for="firstname">Nouveau prénom(Entre 2 à 10 caractères): 
    </label><input type="text" name="firstname"><br>
    <label for="username">Nouveau nom d'utilisateur(Entre 2 à 10 
        caractères): </label><input type="text" name="username" value="<?php if(isset($username)) echo $username; ?>"><br>

    <label for="password">Nouveau mot de passe(Entre 3 à 10 caractères): </label><input type="password" name="password"><br>

    <label for="secret_question">Choisir une question secrete: </label>
    <select name="secret_question" id="secret_question">
      <option value="">--Choisir une option--</option>
      <option value="Quelle est votre couleur préférée?">Quelle est votre 
          couleur préférée?</option>
